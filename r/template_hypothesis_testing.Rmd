...
kickstarter_data <- read.csv('../data/kickstarter_data.csv')[1:1000,]
```
...
```{r, include=TRUE, comment=NA, echo=TRUE}
Y0_mean <- mean((1-kickstarter_data$z)*kickstarter_data$y)
Y1_mean <- mean(kickstarter_data$z*kickstarter_data$y)

V0 <- sum(((1-kickstarter_data$z)*kickstarter_data$y - Y0_mean)^2)/(N-1)
V1 <- sum((kickstarter_data$z*kickstarter_data$y - Y1_mean)^2)/(N-1)
